...
async function getAPI() {
    ...
    myAreaa= await result.json();
    console.log(myAreaa.meals.length);
    displayData();
}
getAPI();
let temp=``;
function displayData(){
    ...
    for (let i = 0; i <myAreaa.meals.length; i++) {
        if(myAreaa.meals[i].strArea!="Unknown"){
            temp+=`<div class="col-lg-3 col-md-6 text-white text-center " >
            <div class="p-3">
            <i class="fa-solid fa-city">
            <h2 class="fs-5 lead text-white">${myAreaa.meals[i].strArea}</h2></i>
    ...
    document.addEventListener('click',async function(e) {
        ...
        for (let i = 0; i <areaMeals.meals.length; i++) {
            ...
            <div class="bg-white position-relative meal rounded-3 ">
              <img src=${areaMeals.meals[i].strMealThumb} alt="" class="w-100 bg-black ">
              <div class="imgOverlay w-100 h-100 d-flex justify-content-center align-items-center ">
                <h2 class="lead fs-5">${areaMeals.meals[i].strMeal}</h2>
              </div>
            </div>
            </div> `
        }
