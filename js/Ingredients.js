...
async function getAPI() {
    let result= await fetch('https://www.themealdb.com/api/json/v1/1/list.php?i=list');
     myIngredients= await result.json();
...
function displayData(){
    for (let i = 0; i <20; i++) {
            temp+=`<div class="col-lg-3 col-md-6 text-white text-center " >
            <div class="p-3">
            <i class="fa-solid fa-bowl-food">
            <h2 class="fs-4 lead text-white">${myIngredients.meals[i].strIngredient}</h2></i>
            <p >${myIngredients.meals[i].strDescription.substring(0,100)}</p>
            </div>
            </div>`
    }
    $('#myIngredients').html(temp);
...
function displayCtiDes(){
    document.addEventListener('click',async function(e) {
        ...
        let result= await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${IngredientName}`)
        ingrediantMeals= await result.json();
        console.log(ingrediantMeals.meals[0].strMealThumb)
        for (let i = 0; i <ingrediantMeals.meals.length; i++) {
            temp+=`<div class="col-lg-3 col-md-6" >
            <div class="bg-white position-relative meal rounded-3 ">
              <img src=${ingrediantMeals.meals[i].strMealThumb} alt="" class="w-100 bg-black ">
              <div class="imgOverlay w-100 h-100 d-flex justify-content-center align-items-center ">
                <h2 class="lead fs-3">${ingrediantMeals.meals[i].strMeal}</h2>
              </div>
            </div>
            </div> `
        }
...
document.addEventListener('click',async function(e) {
    if((e.target).classList.contains('imgOverlay')||(e.target).classList.contains('lead')) {
        ...
        let myHome=window.open('area.html','_self');
